...
市面上的打印方案大致可以分成三类：打印设计器、DOM 打印插件、PDF / 报表生成工具。它们都能解决一部分问题，但关注点不一样。

打印模板工作台更适合 Vue 3 业务系统里的模板化打印：业务人员在前端编辑模板，系统保存模板文档，运行时注入订单、标签、出库单等业务数据，再通过同一套运行时完成预览和浏览器打印。

## 常见插件和方案

| 方案 | 类型 | 典型用途 | 选型提示 |
| --- | --- | --- | --- |
| [`vue-plugin-hiprint`](https://github.com/CcSimple/vue-plugin-hiprint) | hiprint 工具库 | 基于 hiprint API 组装设计器、模板和打印链路 | 生态成熟，适合已有 hiprint 模板或需要直接打印客户端的项目；但它本身更偏 JavaScript 工具库，UI 和业务集成通常需要项目自行组织 |
| [`sv-print`](https://ccsimple.github.io/sv-print-docs/) | 打印设计器组件 | 基于 hiprint 的现成设计器，提供 Svelte、Vue、React 等接入包 | 上手快，适合希望直接使用设计器组件的项目；如果团队技术栈固定在 Vue 3 + Element Plus，需要再评估样式、事件和业务仓储的贴合度 |
| [`vue3-print-nb`](https://www.npmjs.com/package/vue3-print-nb) / [`vue-print-nb`](https://www.npmjs.com/package/vue-print-nb) | Vue 打印指令 | 打印整页、局部 DOM 或指定 URL | 轻量直接，适合“页面已经排好，只差触发打印”的场景；不负责模板设计、模板仓储和复杂版式编辑 |
| [`vue-print-next`](https://www.npmjs.com/package/vue-print-next) | Vue 打印指令 / 方法 | Vue 2 / Vue 3 内容打印、打印预览、忽略指定元素 | 比传统指令插件更贴近 Vue 3 和 TypeScript；核心仍是 DOM 打印，不是模板设计器 |
...
| [`vue-html-to-paper`](https://github.com/mycure-inc/vue-html-to-paper) | Vue HTML 打印插件 | 把 HTML 元素输出到打印窗口 | 适合 Vue 2 时代的简单 HTML 打印；维护活跃度和 Vue 3 适配需要单独评估 |
...
| 维度 | 打印模板工作台的重点 |
| --- | --- |
| 技术栈 | 面向 Vue 3 业务应用，界面交互基于 Element Plus，适合嵌入后台系统 |
| 模板模型 | 使用 `TemplateDocument v1` 保存模板结构，支持校验、迁移、导入、导出和发布载荷生成 |
| 运行时 | 预览和打印共用同一套运行时渲染，减少“预览正常、打印偏差”的问题 |
