...
function tableRowFragments(element) {
  ...
  while (start < rows.length) {
    ...
    while (end + 1 < rows.length) {
      ...
      let groupHeight = 0;
      for (let index = groupStart; index <= groupEnd; index += 1) groupHeight += rowHeight(element, index);
      if (end >= start && usedHeight + groupHeight > availableHeight) break;
      end = groupEnd;
      usedHeight += groupHeight;
      if (usedHeight >= availableHeight) break;
    }
    if (end < start) end = mergedGroupEnd(rows, start);
...
function tablePageCount(element) {
  if (element.type !== "table" || element.props?.autoPaginate === false) return 1;
...
export function paginateRuntimeDocument(document) {
  if (!document) return { pages: [], pageCount: 0 };
