...
function normalizePath(path) {
  return String(path || "")
    .trim()
    .replace(/^@/, "")
    .replace(/\[(?:"([^"]+)"|'([^']+)'|(\d+))\]/g, (_, quoted, singleQuoted, numericIndex) =>
      `.${quoted || singleQuoted || numericIndex}`
    )
    .replace(/^\.+|\.+$/g, "");
}
...
export function applyConstrainedTableTransform(rows, transform) {
  ...
  if (transform.type === "sort" && typeof transform.by === "string") {
    ...
    return {
      rows: source.sort((left, right) =>
        String(left?.[transform.by] ?? "").localeCompare(String(right?.[transform.by] ?? "")) * direction
      ),
      issues: [],
    };
  }
