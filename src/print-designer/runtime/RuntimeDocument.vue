      ...
      >
        ...
        <template v-else-if="element.type === 'table'">
          <table class="runtime-table">
            ...
            <tbody>
              <tr v-for="(row, rowIndex) in tableDisplayRows(element)" :key="rowIndex"><td v-for="column in tableColumns(element)" :key="columnKey(column)" :style="tableCellStyle(element, column, 'body')">{{ tableCellValue(row, column) }}</td></tr>
            </tbody>
...
function tableDisplayRows(element) { const rows = tableRows(element); if (rows.length) return rows; const count = Math.max(1, Math.round(Number(element.editorHints?.rowCount) || 1)); return Array.from({ length: count }, () => null); }
...
function tableCellValue(row, column) { if (row == null) return ''; const result = resolveRelativeRecordPath(row, column?.valuePath || columnKey(column)); return formatTableValue(result.found ? result.value : row?.[columnKey(column)], column?.formatter); }
...
.runtime-placeholder { color: #64748b; font-style: italic; }.runtime-table { width: 100%; height: 100%; border-collapse: collapse; table-layout: fixed; font: inherit; }.runtime-table th, .runtime-table td { overflow: hidden; border: 1px solid currentColor; padding: 2px 4px; text-overflow: ellipsis; white-space: nowrap; }.runtime-label-grid { display: grid; width: 100%; height: 100%; }.runtime-label-cell { display: flex; min-width: 0; flex-direction: column; justify-content: center; overflow: hidden; border: 1px solid currentColor; padding: 2mm; }.runtime-label-cell strong, .runtime-label-cell small { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
