...
<script setup lang="ts">import { computed, defineAsyncComponent } from "vue";
...
const RuntimeBarcode = defineAsyncComponent((): any => import("./RuntimeBarcode.vue")) as any;
const RuntimeQrCode = defineAsyncComponent((): any => import("./RuntimeQrCode.vue")) as any;
const props = defineProps({ document: { type: Object, required: true }, runtimeData: { type: Object, default: (): any => ({}) }, mode: { type: String, default: "preview" } }) as any;
const resolved = computed((): any => resolveRuntimeTemplate(props.document, props.runtimeData)) as any;
const paginated = computed((): any => paginateRuntimeDocument(resolved.value.document)) as any;
const issues = computed((): any => resolved.value.issues || []) as any;
const pageStyle = computed((): any => createRuntimePageStyle(props.document)) as any;
const hasPrintMarks = computed((): any => hasRuntimePrintMarks(props.document)) as any;
function printableElements(page: any): any { return (page.elements || []).filter((element: any): any => element.visible !== false && element.printable !== false); }
function elementStyle(element: any): any { const style = element.style || {}; const isTable = element.type === 'table'; const autoHeight = element.type === 'text' && element.props?.autoHeight === true; const contentHeight = isTable || autoHeight; const vertical = ({ top: 'flex-start', middle: 'center', bottom: 'flex-end' } as any)[style.verticalAlign] || 'flex-start'; const horizontal = ({ left: 'flex-start', center: 'center', right: 'flex-end' } as any)[style.textAlign] || 'flex-start'; return { left: `${element.x || 0}mm`, top: `${element.y || 0}mm`, width: `${element.width || 1}mm`, height: contentHeight ? 'auto' : `${element.height || 1}mm`, minHeight: autoHeight ? `${element.height || 1}mm` : undefined, overflow: contentHeight ? 'visible' : 'hidden', transform: `rotate(${Number(element.rotation) || 0}deg)`, zIndex: Number(element.zIndex) || 0, color: style.color || '#111827', background: style.backgroundColor || 'transparent', fontFamily: style.fontFamily || undefined, fontSize: `${Number(style.fontSize) || 12}px`, fontWeight: style.fontWeight || 'normal', fontStyle: style.fontStyle || 'normal', textDecoration: style.textDecoration || 'none', textAlign: style.textAlign || 'left', lineHeight: style.lineHeight || 1.4, letterSpacing: `${Number(style.letterSpacing) || 0}px`, border: isTable ? '0 solid transparent' : `${Number(style.borderWidth) || 0}px ${style.borderStyle || 'solid'} ${style.borderColor || 'transparent'}`, borderRadius: `${Number(style.borderRadius) || 0}px`, padding: isTable ? '0' : `${Number(style.padding) || 0}mm`, boxSizing: 'border-box', opacity: Number.isFinite(Number(style.opacity)) ? Number(style.opacity) : 1, alignItems: vertical, justifyContent: horizontal, whiteSpace: element.props?.whiteSpace || 'pre-wrap', writingMode: element.props?.writingMode || 'horizontal-tb' }; }
function textValue(value: any, fallback: any): any { return value?.value || (props.mode === "print" ? "" : fallback); }
function valueClass(value: any): any { return value?.status === 'missing' || value?.status === 'empty' ? 'runtime-placeholder' : ''; }
function imageSource(element: any): any { return String(element.runtime?.value?.value || '').trim(); }
function columnKey(column: any): any { return column?.key || ''; }
function tableColumns(element: any): any { return normalizeTableColumns(element.runtime?.table?.columns || []); }
function tableRows(element: any): any { return element.runtime?.table?.rows || []; }
function tableAllRows(element: any): any { return element.runtime?.table?.allRows || tableRows(element); }
function tableDisplayRows(element: any): any {
    const rows = tableRows(element);
    if (rows.length)
        return rows;
    return props.mode === 'print' ? [] : [null];
}
function tableFooterRows(element: any): any { return element.runtime?.table?.footerRows || []; }
function hasBlankTableHeaders(element: any): any {
    if (element.props?.blankHeaders === true)
        return true;
    const columns = tableColumns(element);
    return columns.length > 0 && columns.every((column: any, index: any): any => { const key = String(column?.key || `field${index + 1}`); const title = typeof column?.title === 'string' ? column.title.trim() : ''; return key === `field${index + 1}` && (!title || title === key || /^列\s*\d+$/.test(title)); });
}
function tableHeaderValue(element: any, column: any): any {
    if (hasBlankTableHeaders(element))
        return '';
    return typeof column?.title === 'string' ? column.title : '';
}
function tableColumnStyle(column: any): any { return { width: `${Math.max(1, Number(column?.width) || 1)}px` }; }
function tableRowStyle(element: any, section: any, rowIndex: any): any { const offset = section === 'body' ? Math.max(0, Number(element.runtime?.table?.rowOffset) || 0) : 0; const height = tableRowHeight(element.props, section, rowIndex + offset); return height > 0 ? { height: `${height}mm` } : {}; }
function runtimeTableRawCell(row: any, column: any): any { return row?.[columnKey(column)]; }
function shouldRenderRuntimeTableCell(row: any, column: any): any { return shouldRenderTableCell(runtimeTableRawCell(row, column)); }
function runtimeTableCellRowSpan(row: any, column: any): any { const value = tableCellRowSpan(runtimeTableRawCell(row, column)); return value > 1 ? value : undefined; }
function runtimeTableCellColSpan(row: any, column: any): any { const value = tableCellColSpan(runtimeTableRawCell(row, column)); return value > 1 ? value : undefined; }
function tableCellStyle(element: any, column: any, section: any = 'body', row: any = null): any { const style = element.style || {}; const cellStyle = tableDescriptorStyle(runtimeTableRawCell(row, column)); const textAlign = cellStyle.textAlign || column?.align || (section === 'header' ? style.headerTextAlign : section === 'footer' ? style.footerTextAlign : style.textAlign) || 'left'; const color = cellStyle.color || (section === 'header' ? style.headerColor : section === 'footer' ? style.footerColor : style.color) || 'inherit'; return { textAlign, color, backgroundColor: cellStyle.backgroundColor || undefined, verticalAlign: cellStyle.verticalAlign || style.verticalAlign || 'top', fontSize: cellStyle.fontSize || undefined, fontWeight: cellStyle.fontWeight || undefined, fontStyle: cellStyle.fontStyle || undefined, textDecoration: cellStyle.textDecoration || undefined, borderColor: style.borderColor || 'currentColor', borderWidth: `${Number(style.borderWidth) || 1}px`, borderStyle: style.borderStyle || 'solid', padding: `${Number(style.padding) || 0}mm`, whiteSpace: 'pre-wrap', overflowWrap: 'anywhere', ...cellStyle }; }
function tableCellValue(row: any, column: any, element: any, section: any = 'body'): any {
    if (row == null)
        return '';
...
function tableHeaderStyle(element: any): any { const style = element.style || {}; return { background: style.headerBackgroundColor || style.backgroundColor || 'transparent', color: style.headerColor || style.color || 'inherit', fontSize: `${Number(style.headerFontSize) || Number(style.fontSize) || 12}px`, textAlign: style.headerTextAlign || style.textAlign || 'left' }; }
function tableFooterStyle(element: any): any { const style = element.style || {}; return { background: style.footerBackgroundColor || style.backgroundColor || 'transparent', color: style.footerColor || style.color || 'inherit', fontSize: `${Number(style.footerFontSize) || Number(style.fontSize) || 12}px`, textAlign: style.footerTextAlign || style.textAlign || 'left' }; }
function multiLabelStyle(element: any): any { return { gridTemplateColumns: `repeat(${Math.max(1, Number(element.props?.cols) || 1)}, 1fr)`, gridTemplateRows: `repeat(${Math.max(1, Number(element.props?.rows) || 1)}, 1fr)`, gridAutoFlow: element.props?.direction === 'column' ? 'column' : 'row', gap: `${Number(element.props?.gapY) || 0}mm ${Number(element.props?.gapX) || 0}mm` }; }
function multiLabelItems(element: any): any { const total = Math.max(1, Number(element.props?.rows) || 1) * Math.max(1, Number(element.props?.cols) || 1); const rows = element.runtime?.multiLabel?.rows || []; return Array.from({ length: total }, (_: any, index: any): any => rows[index] ?? null); }
function labelValue(item: any, path: any): any {
    if (item == null)
        return '';
...
function labelPrimary(item: any, element: any, index: any): any {
    if (item == null)
...
function pageNumberValue(element: any, pageRuntime: any): any {
