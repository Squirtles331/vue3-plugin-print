import type { TableColumn, TableRow } from './tableModel.js'

export const TABLE_INSERT_MODES = {
  SAMPLE: 'sample',
  CUSTOM: 'custom',
} as const
export type TableInsertMode = typeof TABLE_INSERT_MODES[keyof typeof TABLE_INSERT_MODES]
...
export function createTableInsertColumns(count = DEFAULT_TABLE_INSERT_COLUMN_COUNT, { mode = DEFAULT_TABLE_INSERT_MODE }: { mode?: TableInsertMode } = {}): TableColumn[] {
  ...
  return Array.from({ length: columnCount }, (_, index): TableColumn => {
...
export function createSampleTableRows(columns: readonly TableColumn[], rowCount = DEFAULT_TABLE_INSERT_ROW_COUNT): TableRow[] {
  ...
  return Array.from({ length: count }, (_, rowIndex) => {
    ...
    return normalizedColumns.reduce<TableRow>((result, column, columnIndex) => {
...
export function createTableSummaryRows(columns: readonly TableColumn[]): TableRow[] {
  ...
  ].map((row: TableRow) => columns.reduce<TableRow>((result, column) => {
    const key = column.key
    result[key] = row[key] || ''
    return result
  }, {}))
}
function createInsertedTableSize(columnCount: number, rowCount: number, footerRowCount: number, { mode, rowHeight }: { mode: TableInsertMode, rowHeight: number }) {
...
export function buildTableInsertOverrides({ mode = DEFAULT_TABLE_INSERT_MODE, columnCount = DEFAULT_TABLE_INSERT_COLUMN_COUNT, rowCount = mode === TABLE_INSERT_MODES.CUSTOM ? DEFAULT_CUSTOM_TABLE_INSERT_ROW_COUNT : DEFAULT_TABLE_INSERT_ROW_COUNT }: { mode?: TableInsertMode, columnCount?: number, rowCount?: number } = {}) {
