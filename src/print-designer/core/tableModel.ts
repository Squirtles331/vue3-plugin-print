...
export interface TableRowHeights { body: Record<string, number>, footer: Record<string, number> }
...
export function toTableCellDescriptor(value: unknown): TableCellDescriptor {
  return isTableCellDescriptor(value) ? normalizeTableCell(value) as TableCellDescriptor : { value: value ?? '' }
}
...
export function updateTableCell(rows: unknown, columns: unknown, rowIndex: number, key: string, value: unknown): TableRow[] {
  ...
  targetRow[key] = isTableCellDescriptor(current)
    ? { ...toTableCellDescriptor(current), value: value ?? '' }
    : value ?? ''
  return nextRows
}
