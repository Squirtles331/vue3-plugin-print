export type TableCellStyleKey = "backgroundColor" | "color" | "fontSize" | "fontWeight" | "fontStyle" | "textAlign" | "verticalAlign" | "textDecoration";
export type TableCellStyle = Partial<Record<TableCellStyleKey, string | number>>;
...
export type TableRowHeights = { body: Record<string, number>; footer: Record<string, number> };
export type TableSection = "body" | "footer";
export interface TableCellSelection { rowIndex?: unknown; colField?: unknown; }
export interface TableCellAnchor { rowIndex: number; columnIndex: number; cell: unknown; }
type UnknownRecord = Record<string, unknown>;

const TABLE_CELL_STYLE_KEYS = new Set<TableCellStyleKey>([
    "backgroundColor",
    "color",
    "fontSize",
    "fontWeight",
...
function spanValue(value: unknown, fallback = 1): number {
    const numeric = Math.floor(Number(value));
    return Number.isFinite(numeric) && numeric >= 0 ? numeric : fallback;
}
export function normalizeTableColumn(column: unknown, index = 0): TableColumn {
    ...
    const key = typeof source.key === "string" && source.key.trim()
        ? source.key.trim()
        : fallback;
    const valuePath = typeof source.valuePath === "string" && source.valuePath.trim()
    ...
    return {
        ...
        ...(isObject(source.formatter) ? { formatter: clone(source.formatter) } : {}),
    };
}
export function normalizeTableColumns(columns: unknown): TableColumn[] {
...
export function isTableCellDescriptor(value: unknown): value is TableCellDescriptor {
    return isObject(value) && ["value", "field", "result", "rowSpan", "colSpan", "style"].some((key) => Object.hasOwn(value, key));
}
export function normalizeTableCellStyle(style: unknown): TableCellStyle {
    if (!isObject(style))
        return {};
    return Object.fromEntries(Object.entries(style).filter(([key, value]) => TABLE_CELL_STYLE_KEYS.has(key as TableCellStyleKey) && (typeof value === "string" || typeof value === "number") && value !== "")) as TableCellStyle;
}
export function normalizeTableCell(value: unknown): unknown {
    if (!isTableCellDescriptor(value)) {
        return value == null ? "" : value;
    }
    const cell: TableCellDescriptor = {};
...
export function toTableCellDescriptor(value: unknown): TableCellDescriptor {
    return isTableCellDescriptor(value) ? normalizeTableCell(value) as TableCellDescriptor : { value: value == null ? "" : value };
}
export function tableCellValue(value: unknown): unknown {
    if (!isTableCellDescriptor(value))
        return value == null ? "" : value;
    return value.value == null ? "" : value.value;
}
export function tableCellStyle(value: unknown): TableCellStyle {
    return isTableCellDescriptor(value) ? normalizeTableCellStyle(value.style) : {};
}
export function tableCellRowSpan(value: unknown): number {
    return isTableCellDescriptor(value) ? spanValue(value.rowSpan, 1) : 1;
}
export function tableCellColSpan(value: unknown): number {
    return isTableCellDescriptor(value) ? spanValue(value.colSpan, 1) : 1;
}
export function shouldRenderTableCell(value: unknown): boolean {
    return !(isTableCellDescriptor(value) && (tableCellRowSpan(value) === 0 || tableCellColSpan(value) === 0));
}
export function tableCellDisplayValue(value: unknown, rows: unknown = []): unknown {
    if (!isTableCellDescriptor(value))
        return value == null ? "" : value;
    ...
    if (field) {
        const total = (Array.isArray(rows) ? rows : []).reduce((sum: number, row) => {
            const candidate = tableCellValue(isObject(row) ? row[field] : undefined);
...
export function createEmptyTableRow(columns: unknown): TableRow {
    return normalizeTableColumns(columns).reduce<TableRow>((row, column) => {
        row[column.key] = "";
        return row;
    }, {});
}
export function normalizeTableRow(row: unknown, columns: unknown): TableRow {
    const source = isObject(row) ? row : {};
    return normalizeTableColumns(columns).reduce<TableRow>((result, column) => {
        result[column.key] = normalizeTableCell(source[column.key]);
        return result;
    }, {});
}
export function normalizeTableRows(rows: unknown, columns: unknown): TableRow[] {
    return (Array.isArray(rows) ? rows : []).map((row) => normalizeTableRow(row, columns));
}
export function ensureTableRow(rows: unknown, columns: unknown, rowIndex: number): TableRow[] {
    const nextRows = normalizeTableRows(rows, columns);
    while (nextRows.length <= rowIndex)
        nextRows.push(createEmptyTableRow(columns));
    return nextRows;
}
export function updateTableCell(rows: unknown, columns: unknown, rowIndex: number, key: string, value: unknown): TableRow[] {
    const nextRows = ensureTableRow(rows, columns, rowIndex);
    const current = nextRows[rowIndex]?.[key];
    const targetRow = nextRows[rowIndex];
    if (!targetRow)
        return nextRows;
    targetRow[key] = isTableCellDescriptor(current)
        ? { ...toTableCellDescriptor(current), value: value == null ? "" : value }
        : value == null ? "" : value;
    return nextRows;
}
export function getTableCellAnchor(rows: unknown, columns: unknown, rowIndex: number, columnIndex: number): TableCellAnchor | null {
    ...
    for (let sourceRowIndex = 0; sourceRowIndex <= rowIndex; sourceRowIndex += 1) {
        for (let sourceColumnIndex = 0; sourceColumnIndex <= columnIndex; sourceColumnIndex += 1) {
            const column = normalizedColumns[sourceColumnIndex];
            const sourceCell = column ? normalizedRows[sourceRowIndex]?.[column.key] : undefined;
...
function boundsFromCells(cells: readonly TableCellSelection[], columns: unknown): { minRow: number; maxRow: number; minColumn: number; maxColumn: number } | null {
    const normalizedColumns = normalizeTableColumns(columns);
...
export function mergeTableCells(rows: unknown, columns: unknown, cells: unknown): { rows: TableRow[]; changed: boolean; reason?: string; bounds?: { minRow: number; maxRow: number; minColumn: number; maxColumn: number } } {
    const normalizedColumns = normalizeTableColumns(columns);
    const nextRows = normalizeTableRows(rows, normalizedColumns);
    const bounds = boundsFromCells(Array.isArray(cells) ? cells.filter(isObject) : [], normalizedColumns);
    ...
    const anchorColumn = normalizedColumns[bounds.minColumn];
    const anchorRow = nextRows[bounds.minRow];
    if (!anchorColumn || !anchorRow)
        return { rows: nextRows, changed: false, reason: "选区超出表格数据范围。" };
    const anchorKey = anchorColumn.key;
    const anchorCell = toTableCellDescriptor(anchorRow[anchorKey]);
    anchorRow[anchorKey] = {
        ...anchorCell,
    ...
    for (let rowIndex = bounds.minRow; rowIndex <= bounds.maxRow; rowIndex += 1) {
        for (let columnIndex = bounds.minColumn; columnIndex <= bounds.maxColumn; columnIndex += 1) {
            ...
            if (!column || !row)
                continue;
            const cell = toTableCellDescriptor(row[column.key]);
            row[column.key] = { ...cell, rowSpan: 0, colSpan: 0 };
        }
    }
    return { rows: nextRows, changed: true, bounds };
}
export function splitTableCell(rows: unknown, columns: unknown, rowIndex: number, colField: string): { rows: TableRow[]; changed: boolean; reason?: string; anchor?: TableCellAnchor } {
    const normalizedColumns = normalizeTableColumns(columns);
    ...
    for (let targetRow = anchor.rowIndex; targetRow < anchor.rowIndex + rowSpan; targetRow += 1) {
        for (let targetColumn = anchor.columnIndex; targetColumn < anchor.columnIndex + colSpan; targetColumn += 1) {
            ...
            if (!key || !row)
                continue;
            const cell = toTableCellDescriptor(row[key]);
            delete cell.rowSpan;
            delete cell.colSpan;
            row[key] = Object.keys(cell).length === 1 && Object.hasOwn(cell, "value") ? cell.value : cell;
        }
    }
    return { rows: nextRows, changed: true, anchor };
}
export function applyTableCellStyle(rows: unknown, columns: unknown, cells: unknown, patch: unknown): { rows: TableRow[]; changed: boolean } {
    const normalizedColumns = normalizeTableColumns(columns);
    const nextRows = normalizeTableRows(rows, normalizedColumns);
    ...
    (Array.isArray(cells) ? cells : []).forEach((selection) => {
    ...
    unique.forEach((coordinate) => {
        ...
        if (rowIndex === undefined || columnIndex === undefined || !Number.isFinite(rowIndex) || !Number.isFinite(columnIndex))
            return;
        const column = normalizedColumns[columnIndex];
        const row = nextRows[rowIndex];
        if (!column || !row)
        ...
        const cell = toTableCellDescriptor(row[key]);
        row[key] = { ...cell, style: { ...normalizeTableCellStyle(cell.style), ...stylePatch } };
    });
    return { rows: nextRows, changed: unique.size > 0 };
}
export function normalizeTableRowHeights(value: unknown): TableRowHeights {
    const source = isObject(value) ? value : {};
    const section = (key: TableSection): Record<string, number> => Object.fromEntries(Object.entries(isObject(source[key]) ? source[key] : {})
        .map(([index, height]) => [String(index), Number(height)] as const)
        .filter(([index, height]) => /^\d+$/.test(index) && Number.isFinite(height) && height > 0));
    return { body: section("body"), footer: section("footer") };
}
export function updateTableRowHeight(rowHeights: unknown, section: string, rowIndex: unknown, height: unknown): TableRowHeights {
    const next = normalizeTableRowHeights(rowHeights);
    const numeric = Number(height);
    if (section !== "body" && section !== "footer" || !Number.isFinite(numeric) || numeric <= 0)
        return next;
    next[section][String(Math.max(0, Math.floor(Number(rowIndex) || 0)))] = numeric;
    return next;
}
function shiftRowHeights(rowHeights: unknown, section: TableSection, index: number, offset: number): TableRowHeights {
    const next = normalizeTableRowHeights(rowHeights);
...
export function insertTableRow(rows: unknown, columns: unknown, rowIndex: unknown, rowHeights: unknown, section: TableSection = "body"): { rows: TableRow[]; rowHeights: TableRowHeights; index: number } {
    const normalizedColumns = normalizeTableColumns(columns);
    const nextRows = normalizeTableRows(rows, normalizedColumns);
...
export function removeTableRow(rows: unknown, columns: unknown, rowIndex: unknown, rowHeights: unknown, section: TableSection = "body"): { rows: TableRow[]; rowHeights: TableRowHeights; changed: boolean } {
    const normalizedColumns = normalizeTableColumns(columns);
    const nextRows = normalizeTableRows(rows, normalizedColumns);
    ...
    normalizedColumns.forEach((column) => {
        for (let sourceRowIndex = 0; sourceRowIndex < nextRows.length; sourceRowIndex += 1) {
            ...
            const nextRow = nextRows[index + 1];
            if (sourceRowIndex === index && span > 1 && nextRow) {
                const promoted: TableCellDescriptor = { ...toTableCellDescriptor(cell), rowSpan: span - 1 };
                nextRow[column.key] = promoted;
            }
            else if (sourceRowIndex < index && sourceRowIndex + span > index) {
                const nextCell: TableCellDescriptor = { ...toTableCellDescriptor(cell), rowSpan: Math.max(1, span - 1) };
                if (nextCell.rowSpan === 1)
                    delete nextCell.rowSpan;
                const sourceRow = nextRows[sourceRowIndex];
                if (sourceRow)
                    sourceRow[column.key] = nextCell;
            }
        }
    });
...
function nextColumnKey(columns: readonly TableColumn[]): string {
...
export function insertTableColumn(columns: unknown, sampleData: unknown, footerData: unknown, columnIndex: unknown): { columns: TableColumn[]; sampleData: TableRow[]; footerData: TableRow[]; index: number } {
    ...
    const insert = (rows: unknown): TableRow[] => normalizeTableRows(rows, sourceColumns).map((row) => {
        const next: TableRow = {};
        nextColumns.forEach((column, currentIndex) => {
            if (currentIndex === index) {
                ...
                next[column.key] = normalizeTableCell(original ? row[original.key] : undefined);
            }
        });
...
export function removeTableColumn(columns: unknown, sampleData: unknown, footerData: unknown, columnIndex: unknown): { columns: TableColumn[]; sampleData: TableRow[]; footerData: TableRow[]; changed: boolean; removed?: TableColumn } {
    const normalizedColumns = normalizeTableColumns(columns);
    ...
    const remove = (rows: unknown): TableRow[] => normalizeTableRows(rows, normalizedColumns).map((row) => {
        const source = { ...row };
        normalizedColumns.forEach((column, sourceIndex) => {
            ...
            if (sourceIndex === index && span > 1) {
                ...
                const nextCell: TableCellDescriptor = { ...toTableCellDescriptor(cell), colSpan: Math.max(1, span - 1) };
...
export function renameTableColumn(columns: unknown, sampleData: unknown, footerData: unknown, columnIndex: unknown, nextKey: unknown, nextTitle: unknown): { columns: TableColumn[]; sampleData: TableRow[]; footerData: TableRow[] } | { error: string } | null {
    const normalizedColumns = normalizeTableColumns(columns);
    const index = Math.floor(Number(columnIndex));
    ...
    const nextColumns = normalizedColumns.map((column, currentIndex) => currentIndex === index
        ...
        : column);
    const rename = (rows: unknown): TableRow[] => normalizeTableRows(rows, normalizedColumns).map((row) => {
        const next: TableRow = Object.fromEntries(Object.entries(row).map(([cellKey, cellValue]) => {
...
export function tableRowHeight(props: unknown, section: "body" | "footer" | "header", rowIndex: unknown): number {
    const source = isObject(props) ? props : {};
    const heights = normalizeTableRowHeights(source.rowHeights);
    if (section === "body")
        return Number(heights.body[String(rowIndex)]) || Number(source.rowHeight) || 0;
    if (section === "footer")
        return Number(heights.footer[String(rowIndex)]) || Number(source.footerHeight) || 0;
    return Number(source.headerHeight) || 0;
}
