...
import type { ElementProperties, ElementStyle, PrintElementType, TemplateElement } from "../types.js";

export type ElementOverrides = Partial<TemplateElement>;
export interface ElementDefinition {
    label: string;
    createDefaults: () => ElementOverrides;
    inspectorSchema: unknown;
    interactionPolicy: "box";
    renderer: PrintElementType;
}

const tableStyle: ElementStyle = {
    fontSize: 12,
    color: "#111827",
...
export const ELEMENT_DEFINITIONS: Record<PrintElementType, ElementDefinition> = {
    [ElementType.TEXT]: {
        label: "文本",
...
export const ELEMENT_PALETTE = Object.entries(ELEMENT_DEFINITIONS).map(([type, definition]) => ({ type: type as PrintElementType, label: definition.label }));
export function getElementDefinition(type: unknown): ElementDefinition | null {
    return typeof type === "string" && Object.hasOwn(ELEMENT_DEFINITIONS, type) ? ELEMENT_DEFINITIONS[type as PrintElementType] : null;
}
export function isElementType(type: unknown): type is PrintElementType { return getElementDefinition(type) !== null; }
