...
// 单位换算：px -> 目标单位
export function pxToUnit(px: number, unit: keyof typeof PX_PER_UNIT): number {
  return px / (PX_PER_UNIT[unit] || 1)
}
// 单位换算：目标单位 -> px
export function unitToPx(value: number, unit: keyof typeof PX_PER_UNIT): number {
...
// 条码支持格式
export const BARCODE_FORMATS = ['CODE128', 'CODE39', 'EAN13', 'EAN8', 'UPC', 'ITF14', 'MSI'] as const
// 二维码纠错档位
export const QRCODE_ECC_LEVELS = ['L', 'M', 'Q', 'H'] as const
// 页码格式模板
export const PAGE_NUMBER_FORMATS = ['1', 'Page 1', '1/N', '第1页', '第1页/共N页'] as const
// 历史记录动作类型（用于历史面板显示）
