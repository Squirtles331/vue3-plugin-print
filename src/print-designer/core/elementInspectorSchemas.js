...
function createField(key, label, source, control, options = {}) {
  return {
    key,
    label,
    source,
...
function createSection(key, label, layout, fields = [], options = {}) {
  return {
    key,
    label,
    layout,
...
function createTab(key, sections = []) {
  return {
    key,
    sections,
  };
...
function findSchemaSection(schema, tabKey, sectionKey) {
  return schema?.tabs?.find((tab) => tab.key === tabKey)?.sections?.find((section) => section.key === sectionKey) || null;
}

function patchSchemaFieldControl(schema, tabKey, sectionKey, source, key, control) {
...
function insertSchemaFieldAfter(schema, tabKey, sectionKey, afterKey, field) {
  const section = findSchemaSection(schema, tabKey, sectionKey);
...
function removeSchemaField(schema, tabKey, sectionKey, source, key) {
  const section = findSchemaSection(schema, tabKey, sectionKey);
...
function createCommonInspectorSchema(propertyFields = [], styleFields = [], advancedFields = []) {
  return {
    tabs: [
...
export const CIRCLE_INSPECTOR_SCHEMA = createCommonInspectorSchema(
  [...GEOMETRY_FIELDS],
  [
    createField("opacity", "透明度", FIELD_SOURCE.STYLE, FIELD_CONTROL.NUMBER, {
      min: 0,
...
export const MULTI_LABEL_INSPECTOR_SCHEMA = createCommonInspectorSchema(
  ...
  [...BASE_TEXT_STYLE_FIELDS, ...BASE_STYLE_FIELDS],
  [...BASE_ADVANCED_FIELDS]
);

patchSchemaFieldControl(
...
insertSchemaFieldAfter(
  PAGE_NUMBER_INSPECTOR_SCHEMA,
  INSPECTOR_TABS.PROPERTY,
  "property",
  "rotation",
  createField("content", "页码示例", FIELD_SOURCE.ROOT, FIELD_CONTROL.INPUT)
);
insertSchemaFieldAfter(
  PAGE_NUMBER_INSPECTOR_SCHEMA,
  INSPECTOR_TABS.PROPERTY,
  "property",
  "content",
  createField("totalPages", "总页数", FIELD_SOURCE.PROPS, FIELD_CONTROL.NUMBER, {
    min: 1,
    step: 1,
  })
);
...
insertSchemaFieldAfter(
  ...
  "property",
  "format",
  createField("displayValue", "显示文字", FIELD_SOURCE.PROPS, FIELD_CONTROL.SWITCH)
);
