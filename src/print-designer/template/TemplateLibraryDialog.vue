    ...
    <div class="template-library">
      <div class="template-library__toolbar">
        ...
        <div class="template-library__actions">
          <PdButton type="danger" plain :disabled="loading || !templates.length" @click="confirmClear">
            清空本地库
          </PdButton>
          <PdButton :loading="loading" @click="emit('refresh')">
            刷新
          </PdButton>
        </div>
      </div>
      <p class="template-library__summary">
        {{ summaryText }}
      </p>
      <p v-if="!loading && !filteredTemplates.length" class="template-library__empty">
        {{ emptyText }}
      </p>
      <article v-for="template in filteredTemplates" :key="template.id" class="template-library__item">
        ...
        <PdButton text type="danger" class="template-library__delete" @click="confirmRemove(template)">
          删除
        </PdButton>
      </article>
    </div>
