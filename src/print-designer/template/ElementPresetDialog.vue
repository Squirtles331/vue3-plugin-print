    ...
    <div class="preset-dialog">
      ...
      <article v-for="preset in presets" :key="preset.id" class="preset-dialog__item">
        ...
        <div class="preset-dialog__actions">
          <PdButton type="primary" size="small" @click="emit('insert', preset.id)">插入</PdButton>
          <PdButton size="small" @click="emit('rename', preset)">重命名</PdButton>
          <PdButton type="danger" plain size="small" @click="emit('remove', preset)">删除</PdButton>
        </div>
      </article>
    </div>
  </PdDialog>
