...
function textElement(page: any, overrides: any): any {
    return createElement("text", { pageId: page, ...overrides });
}
function tableElement(page: any, overrides: any): any {
    return createElement("table", { pageId: page, ...overrides });
}
function barcodeElement(page: any, overrides: any): any {
    return createElement("barcode", { pageId: page, ...overrides });
}
function multiLabelElement(page: any, overrides: any): any {
    return createElement("multiLabel", { pageId: page, ...overrides });
}
function createCatalogDocument({ name, paper, elements = [] }: any): any {
    const id = pageId() as any;
...
const STARTER_DEFINITIONS = [
    {
        ...
        create: (): any => createCatalogDocument({
            name: "订单摘要",
            paper: { preset: "A4", widthMm: 210, heightMm: 297 },
            elements: (page: any): any => [
    ...
    },
    {
        ...
        create: (): any => createCatalogDocument({ name: "自定义空白页", paper: { preset: "A4", widthMm: 210, heightMm: 297 } }),
    },
] as any;
export const STARTER_TEMPLATE_CATALOG = Object.freeze(STARTER_DEFINITIONS.map(({ create, ...definition }: any): any => Object.freeze(definition))) as any;
export function listStarterTemplates(category: any = ""): any {
    return STARTER_TEMPLATE_CATALOG.filter((template: any): any => !category || template.category === category).map((template: any): any => ({ ...template, thumbnail: { ...template.thumbnail, blocks: template.thumbnail.blocks.map((block: any): any => ({ ...block })) } }));
}
export function instantiateStarterTemplate(id: any): any {
    const definition = STARTER_DEFINITIONS.find((template: any): any => template.id === id) as any;
    if (!definition) {
