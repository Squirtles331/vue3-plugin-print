...
<template>
  <section class="layout-toolbar" aria-label="布局命令">
    ...
    <div class="layout-toolbar__group">
      <button type="button" :disabled="!hasEditableSelection" title="复制选中的未锁定元素" @click="duplicate">
        复制
      </button>
      <button type="button" :disabled="!canArrange" title="置于顶层" @click="order('front')">
        置顶
      </button>
      <button type="button" :disabled="!canArrange" title="置于底层" @click="order('back')">
        置底
      </button>
    </div>
    <div class="layout-toolbar__group">
    ...
    <div class="layout-toolbar__group">
      <button type="button" :disabled="!canDistribute" title="水平分布（保持最外侧元素位置）" @click="distribute('horizontal')">
        横分
      </button>
      <button type="button" :disabled="!canDistribute" title="垂直分布（保持最外侧元素位置）" @click="distribute('vertical')">
        纵分
      </button>
    </div>
