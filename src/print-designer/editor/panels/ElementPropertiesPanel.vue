...
import { FIELD_CONTROL, INSPECTOR_TABS, SECTION_LAYOUT, TAB_LABELS } from '../../core/elementInspectorSchemas'
...
function tableJsonPropValue(key) {
  if (key === 'columns') {
    return JSON.stringify(tableColumnsValue(tableColumnsField).map((column) => {
      const result = {
        field: column.key,
...
function updateTableColumn(field, index, prop, value) {
  ...
  const nextColumns = columns.map((column, columnIndex) => columnIndex === index ? normalizeTableColumn({ ...column, [prop]: nextValue }, columnIndex) : column)
  if (prop === 'key') {
...
function updateTableColumnFormatter(field, index, key, value) {
  ...
  const nextColumns = columns.map((column, columnIndex) => columnIndex === index ? normalizeTableColumn(nextColumn, columnIndex) : column)
  setTableColumns(field, nextColumns)
}
...
function normalizeTableSampleRow(row, columns) {
  ...
  return columns.reduce((result, column) => {
    result[column.key] = source[column.key] ?? ''
    return result
  }, {})
}
...
function removeTableSampleRow(field, rowIndex) {
  setFieldValue(field, tableSampleRowsValue(field).filter((_, index) => index !== rowIndex))
}
...
function removeTableFooterRow(field, rowIndex) {
  setFieldValue(field, tableFooterRowsValue(field).filter((_, index) => index !== rowIndex))
}
...
function updateMultiLabelSampleCell(field, index, key, value) {
  const nextData = multiLabelSampleDataValue(field).map((item, itemIndex) => itemIndex === index ? { ...item, [key]: value == null ? '' : String(value) } : item)
  ...
  <div class="element-properties-panel" :class="{ 'element-properties-panel--table': isTableObject }">
    <section v-if="isMultipleSelection" class="element-properties-panel__multi">
      <header class="element-properties-panel__summary">
        ...
        <p class="element-properties-panel__summary-hint">
          仅显示所有已选元素共享的基础属性；数据绑定和元素专属配置请单独编辑。
        </p>
      </header>
        ...
        <div class="element-properties-panel__section-body is-grid-2">
          ...
          <div class="element-properties-panel__field">
            ...
            <div class="element-properties-panel__action-grid">
              <button type="button" class="element-properties-panel__section-button" @click="reorderMultipleSelection('front')">
                置于顶层
              </button>
              <button type="button" class="element-properties-panel__section-button" @click="reorderMultipleSelection('back')">
                置于底层
              </button>
            </div>
          </div>
        ...
        <template v-if="activeTab === INSPECTOR_TABS.PROPERTY">
          ...
          <section class="element-properties-panel__section">
            ...
            <div class="element-properties-panel__section-body is-stack">
              ...
              <p class="element-properties-panel__field-help">
                仅支持受限 JSON 配置，不支持任意 JavaScript。
              </p>
