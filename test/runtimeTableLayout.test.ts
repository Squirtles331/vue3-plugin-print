...
it('summary tokens format page and document totals from shared table helpers', () => {
...
it('runtime table summaries use fragment rows for page totals and all rows for grand totals', () => {
  const sampleData = [
...
it('runtime table uses a single preview placeholder row and omits it when printing', () => {
  const document = createTableDocument({ columns: [{ key: 'name' }], sampleData: [], footerData: [], showFooter: false, autoPaginate: true })
...
it('runtime pagination keeps a merged row group in one table fragment', () => {
  const document = createTableDocument({
