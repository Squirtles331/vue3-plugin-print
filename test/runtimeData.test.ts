...
it('resolves nested runtime JSON paths', () => {
...
it('resolves bound elements and does not substitute sample rows for a missing binding', () => {
...
it('resolves a reactive template without mutating the source document', () => {
...
it('preserves explicit design preview rows when no runtime binding is configured', () => {
  const document = createBlankTemplateDocument({
...
it('keeps empty labels structural and leaves unbound machine-readable values empty', () => {
  const document = createBlankTemplateDocument({
    pages: [{ id: 'page-1', title: 'Page 1', elements: [
...
it('only executes declarative table transforms and reports invalid transforms', () => {
...
it('validates print runtime with the same blocking issues used by preview', () => {
  const document = createBlankTemplateDocument({
    pages: [{ id: 'page-1', title: 'Page 1', elements: [
...
it('runtime data cannot replace authored table columns or presentation', () => {
  const document = {
    schemaVersion: 2,
...
it('paginates table rows deterministically and exposes page totals', () => {
  const document = createBlankTemplateDocument({
...
it('table footer repeat is deterministic across generated pages', () => {
  const document = createBlankTemplateDocument({
...
it('strict preflight blocks incomplete bindings and supports an explicit opt-out', () => {
  const document = createBlankTemplateDocument({
...
it('strict preflight blocks printable elements outside the authored safe area', () => {
  const document = createBlankTemplateDocument({
    pages: [{ id: 'page-1', title: 'Page 1', elements: [
...
it('pagination keeps only marked elements and omits short table fragments', () => {
...
it('derives safe binding paths from runtime JSON', () => {
...
it('describes scalar, object, and array paths for safe editor binding', () => {
