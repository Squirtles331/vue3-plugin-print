...
it('property matrix covers the page and every supported element type', () => {
...
it('normalizes v2 editor hints, mappings, and options without losing unknown safe props', () => {
  const source = {
    schemaVersion: 2,
    id: 'property-parity',
  ...
  const table = serialized.document.pages[0].elements[0]
  const labels = serialized.document.pages[0].elements[1]
...
it('safe property semantics are deterministic and reject unsupported formatter values', () => {
...
it('runtime data never overrides a template\'s static geometry or style', () => {
...
it('normalization keeps a circle circular and stores opacity in print style', () => {
