...
it('table model merges, splits and styles descriptor cells without mutating source rows', () => {
...
it('table model keeps data, spans and row-height indexes aligned through structural edits', () => {
...
it('renaming a column follows cell values and safe footer summaries', () => {
  const renamed = renameTableColumn(columns, [{ name: 'A', amount: 5 }], [{ name: 'Total', amount: { value: 'Σ ', field: 'amount' } }], 1, 'price', 'Price')
