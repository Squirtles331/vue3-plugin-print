...
function createTextObject(overrides: any = {}): any {
    return {
        id: "text-1",
        type: "text",
        content: "Draft text",
...
function createMountedText(object: any, { selected = true, runtimeData = {} }: any = {}): any {
    const pinia = createPinia() as any;
    ...
    const wrapper = mount(TextElement, { props: { object }, attachTo: document.body, global: { plugins: [pinia] } }) as any;
    return { wrapper, documentStore, historyStore, previewStore, selectionStore };
}
test("text element resolves bound preview data without changing the authored fallback", async (): Promise<any> => {
    const object = createTextObject({
        content: "Fallback title",
        variable: "invoice.customer.name",
        props: { sampleValue: "Sample customer" },
    }) as any;
    const { wrapper, previewStore } = createMountedText(object, { selected: false, runtimeData: { invoice: { customer: { name: "Ada" } } } }) as any;
...
test("selected text commits the authored fallback once and keeps runtime data read-only", async (): Promise<any> => {
    const object = createTextObject({ content: "Fallback", variable: "customer.name" }) as any;
    const { wrapper, documentStore, historyStore, previewStore } = createMountedText(object, { runtimeData: { customer: { name: "Ada" } } }) as any;
    await wrapper.find(".pd-text-element__value").trigger("dblclick");
    const editor = wrapper.find("textarea") as any;
...
test("text inline editing preserves Shift+Enter and cancels with Escape", async (): Promise<any> => {
    const object = createTextObject() as any;
    const { wrapper, documentStore, historyStore } = createMountedText(object) as any;
    await wrapper.find(".pd-text-element__value").trigger("dblclick");
    const editor = wrapper.find("textarea") as any;
...
test("text inline editing commits on blur", async (): Promise<any> => {
    const object = createTextObject() as any;
    const { wrapper, documentStore, historyStore } = createMountedText(object) as any;
    await wrapper.find(".pd-text-element__value").trigger("dblclick");
    const editor = wrapper.find("textarea") as any;
...
test("text inline editing saves when its selection changes", async (): Promise<any> => {
    const object = createTextObject() as any;
    const other = createTextObject({ id: "text-2" }) as any;
    const { wrapper, documentStore, selectionStore } = createMountedText(object) as any;
...
test("quick toolbar formats a single unlocked text element and stays hidden for locked text", async (): Promise<any> => {
    const object = createTextObject() as any;
    const { wrapper, documentStore, historyStore } = createMountedText(object) as any;
    await nextTick();
    const boldButton = document.body.querySelector("button[aria-label='粗体']") as any;
    ...
    const writingButton = document.body.querySelector("button[aria-label='竖排文字']") as any;
    assert.ok(writingButton);
    writingButton.click();
    await nextTick();
    assert.equal(documentStore.objectsById[object.id].props.writingMode, "vertical-rl");
    await wrapper.setProps({ object: documentStore.objectsById[object.id] });
    const fontSizeInput = document.body.querySelector("input[aria-label='字号']") as any;
    ...
    wrapper.unmount();
    const locked = createTextObject({ id: "text-locked", locked: true }) as any;
    const mountedLocked = createMountedText(locked) as any;
    await nextTick();
...
test("paper canvas gives only selected unlocked text elements direct content interaction", async (): Promise<any> => {
    const object = createTextObject() as any;
    const pinia = createPinia() as any;
    setActivePinia(pinia);
    const documentStore = useEditorDocumentStore() as any;
    ...
    documentStore.addObject(object);
    selectionStore.select(object.id);
    const wrapper = mount(PaperCanvas, { props: { pixelsPerUnit: 3.78 }, global: { plugins: [pinia] } }) as any;
    await nextTick();
