...
it('locked elements reject property, order, and delete mutations until explicitly unlocked', () => {
...
it('all supported data-aware elements resolve runtime values and preserve structural elements', () => {
  const pageId = 'page-1'
  const elements = [
...
it('property capability validation rejects invalid machine-code, binding, table, and label values', () => {
...
it('unresolved bindings stay explicit and removed table scripts are rejected', () => {
  const pageId = 'page-1'
