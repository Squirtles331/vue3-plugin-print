...
function createMountedText(object, { selected = true, runtimeData = {} } = {}) {
  ...
  if (selected) selectionStore.select(object.id);
...
test("text element resolves bound preview data without changing the authored fallback", async () => {
...
test("selected text commits the authored fallback once and keeps runtime data read-only", async () => {
...
test("text inline editing preserves Shift+Enter and cancels with Escape", async () => {
  const object = createTextObject();
...
test("text inline editing commits on blur", async () => {
  const object = createTextObject();
  const { wrapper, documentStore, historyStore } = createMountedText(object);
...
test("text inline editing saves when its selection changes", async () => {
  const object = createTextObject();
...
test("quick toolbar formats a single unlocked text element and stays hidden for locked text", async () => {
  const object = createTextObject();
  const { wrapper, documentStore, historyStore } = createMountedText(object);
  await nextTick();
...
test("paper canvas gives only selected unlocked text elements direct content interaction", async () => {
  const object = createTextObject();
  const pinia = createPinia();
  setActivePinia(pinia);
  const documentStore = useEditorDocumentStore();
