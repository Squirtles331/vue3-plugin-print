...
it('print CSS uses the authored paper size with the full-sheet coordinate system', () => {
...
it('browser print prepares runtime-only output inside an isolated iframe', async () => {
...
it('browser print waits for machine-readable output before opening the print dialog', async () => {
  const originalAppendChild = document.body.appendChild.bind(document.body)
  let printFrame = null
  const print = vi.fn()
  vi.spyOn(document.body, 'appendChild').mockImplementation((node) => {
...
it('browser print fails clearly and cleans up when image assets time out', async () => {
  const template = createBlankTemplateDocument({
    pages: [{
      id: 'page-1',
      title: 'Page 1',
