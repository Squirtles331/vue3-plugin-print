...
test("table element renders semantic columns and preserves merged cell metadata", (): any => {
    ...
    const wrapper = mountTable(object) as any;
...
test("table element keeps bindings and resolves formatted nested values", (): any => {
    const object = createTableObject({
        props: {
    ...
    assert.deepEqual(wrapper.findAll(".pd-table-element__bindings span").map((token: any): any => token.text()), [
        "数据：{{invoice.items}}",
        "页脚：{{invoice.summary}}",
    ]);
    wrapper.unmount();
});
test("custom blank-table preset renders as a headerless 5 by 10 grid", (): any => {
    const custom = buildTableInsertOverrides({ mode: TABLE_INSERT_MODES.CUSTOM }) as any;
    const object = createTableObject({ props: custom.props, editorHints: custom.editorHints }) as any;
    const wrapper = mountTable(object) as any;
...
test("selected table edits only design sample data and creates an undoable command", async (): Promise<any> => {
    const object = createTableObject({
    ...
    const wrapper = mount(TableElement, { props: { object }, global: { plugins: [pinia] } }) as any;
