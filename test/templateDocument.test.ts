...
it('new elements reserve business data for table line-item presets only', () => {
...
it('custom table insert keeps structural dimensions without sample data', () => {
...
it('default custom table is a 5 by 10 headerless grid sized like the blank-grid preset', () => {
...
it('normalizes v2 table data into the safe editable cell model', () => {
...
it('template serialization strips editor-only page state', () => {
...
it('local repository persists normalized templates', async () => {
...
it('local repository deletes saved templates and clears only its own collection', async () => {
  const memory = new Map()
  const storage = { getItem: key => memory.get(key) || null, setItem: (key, value) => memory.set(key, value) }
...
it('local repository rejects v1 records when they are read', async () => {
  const memory = new Map()
  const storage = { getItem: key => memory.get(key) || null, setItem: (key, value) => memory.set(key, value) }
...
it('rejects v1 templates instead of migrating them', () => {
...
it('rejects removed v1 field aliases even when a document claims schema v2', () => {
  const result = validateTemplateDocument({
    schemaVersion: 2,
...
it('normalizes v2 same-page groups and reports invalid group constraints', () => {
...
it('runtime data drafts are isolated per template and never enter template JSON', async () => {
  const memory = new Map()
  const storage = { getItem: key => memory.get(key) || null, setItem: (key, value) => memory.set(key, value) }
...
it('local repository surfaces corrupt browser storage', async () => {
  const storage = { getItem: () => 'not-json', setItem: () => { } }
  const repository = createLocalTemplateRepository({ storage })
  await assert.rejects(() => repository.list(), /Unable to read local template storage/)
})
it('publish-ready payload only exposes runtime template fields', () => {
...
it('rejects a non-v2 document unit', () => {
  const result = validateTemplateDocument({
    schemaVersion: 2,
...
it('reports bounded invalid values without serializing invalid output', () => {
  const template = createBlankTemplateDocument()
...
it('property capability registry declares runtime-backed type-specific fields', () => {
...
it('reports unsupported reactive elements without throwing clone errors', () => {
