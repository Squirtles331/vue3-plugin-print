...
const repositoryName = "vue3-plugin-print" as any;
const docsDist = resolve("docs", ".vitepress", "dist") as any;
const docsIndexPath = resolve(docsDist, "index.html") as any;
const demoIndexPath = resolve(docsDist, "demo", "index.html") as any;
function resolveExpectedBasePath(): any {
    const configuredBase = process.env.VITEPRESS_BASE || process.env.VITE_BASE_URL as any;
    if (typeof configuredBase === "string" && configuredBase.trim()) {
        const normalizedBase = configuredBase.trim().replace(/^\/+|\/+$/g, "") as any;
        return normalizedBase ? `/${normalizedBase}/` : "/";
    }
    return `/${repositoryName}/`;
}
function joinBase(base: any, child: any): any {
    const normalizedBase = base.replace(/\/+$/, "") as any;
    return `${normalizedBase}/${child.replace(/^\/+|\/+$/g, "")}/`;
}
const expectedBasePath = resolveExpectedBasePath() as any;
...
try {
    ...
    const docsIndex = readFileSync(docsIndexPath, "utf8") as any;
    const demoIndex = readFileSync(demoIndexPath, "utf8") as any;
